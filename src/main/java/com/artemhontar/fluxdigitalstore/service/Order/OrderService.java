...
public class OrderService {
    ...
    public OrderService(FinanceService financeService, AuthenticationService authenticationService, OrderRepo orderRepo, InventoryService inventoryService, OrderConverter orderConverter, BookService bookService, DeliveryAddressService deliveryAddressService) {
    ...
    public UserOrder createOrder(UserOrderRequest orderRequest) {
        ...
        try {
            ...
            log.error("Order creation failed: User not authenticated or address not found.", e);
            // Catching NotFoundException here handles the case where the user is not logged in OR an address couldn't be found
            if (order != null) {
                // If order was partially created (unlikely, but safe), cancel it.
     ...
     *
     * @param orderRequest  The request containing order items and delivery address ID.
     ...
     * @throws NotFoundException If the current user is not logged in or a valid address is not found.
     */
    private UserOrder generateOrder(UserOrderRequest orderRequest, String paymentID, ORDER_STATUS initialStatus) {
        ...
        if (opUser.isPresent()) {
            ...
            // 1. Get Delivery Address using the dedicated service logic
            DeliveryAddress address = deliveryAddressService.getDeliveryAddress(orderRequest, user)
                    .orElseThrow(() -> {
                        log.error("Order creation failed: No valid delivery address found for User ID {}.", user.getId());
                        return new NotFoundException("A valid delivery address must be provided or saved to the user entity.");
                    });

            // 2. Assign the found address to the order
            order.setDeliveryAddress(address);
            log.debug("Delivery address ID {} assigned to the order.", address.getId());

            // 3. Convert DTO items to OrderItem entities
            for (OrderItemDTO itemDTO : orderRequest.getOrderItems()) {
                OrderItem item = orderConverter.convertToEntity(itemDTO);
    ...
    public void validateOrderRequest(UserOrderRequest orderRequest) {
        if (orderRequest == null || orderRequest.getOrderItems() == null) {
            throw new IllegalArgumentException("orderRequest cannot be null or empty");
        }
        for (OrderItemDTO item : orderRequest.getOrderItems()) {
            if (!bookService.existsByID(item.getProductId())) {
